...
def handle_missing_values(df, method):
    if method == 'mean':
        print("mean")
        # For numerical columns, impute using mean
        numerical_imputer = SimpleImputer(strategy='mean')
        numerical_columns = df.select_dtypes(include=['number']).columns
        # print(numerical_columns.empty)
...
def scalling(df, method):
    ...
    try:
        ...
        # Initialize scaler based on the chosen method
        if method == 'standard':
        ...
        # Create a DataFrame with scaled numeric columns
        df_scaled_numeric = pd.DataFrame(scaled_data, columns=df_numeric.columns)

        # Get non-numeric columns from the original DataFrame
...
def detect_outliers_standard_deviation(data, threshold=3):
    try:
        ...
        if np.isnan(std_deviation) or std_deviation == 0:
            # If standard deviation is 0 or NaN, return the original data
            return data
        else:
...
def read_excel_file(file: UploadFile, sheet_name: Union[int, str] = 0):
    """
    Read Excel file from UploadFile object and return DataFrame.
    """
    contents = file.file.read()
...
async def handle(file: UploadFile = File(...),mode:str=Form(...)):
    try:
        ...
        # print("delimit",csv_file.delimit)
        # save_file(csv_file.path, file_csv,csv_file.delimit)
...
async def scale(file: UploadFile = File(...),mode:str=Form(...)):
    try:
        ...
        # save_file(csv_file.path, file_csv)
        save_file_processed(df,file_csv)
